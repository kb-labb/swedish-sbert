...
translations = {
    src_lang: {trg_lang: {} for trg_lang in target_languages} for src_lang in source_languages
}
...
for src_lang in source_languages:
    for trg_lang in target_languages:
        ...
        if len(train_sentences) > 0:
            with gzip.open(
                os.path.join(
                    output_folder, "Tatoeba-{}-{}-train.tsv.gz".format(src_lang, trg_lang)
                ),
                "wt",
                encoding="utf8",
            ) as fOut:
